...
void DS1307::setDateTime(uint16_t year, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute, uint8_t second)
{
    ...
    //HAL_I2C_Master_Transmit(hi2c1, DS1307_ADDRESS, , 1, 100);
    #if ARDUINO >= 100
...
void DS1307::setDateTime(uint32_t t)
{
    ...
    for (month = 1; ; ++month)
    {
        uint8_t daysPerMonth = daysArray[month-1];//pgm_read_byte(daysArray + month - 1);
...
void DS1307::readPacket(uint8_t offset, uint8_t * buff, uint8_t size)
{
    ...
    #else
    i2c_write(DS1307_REG_RAM + offset);
    #endif

    //Wire.endTransmission();

    i2c_read(buff, (int)size);
...
uint8_t DS1307::daysInMonth(uint16_t year, uint8_t month)
{
    ...
    days = daysArray[month-1];//pgm_read_byte(daysArray + month - 1);
...
uint16_t DS1307::date2days(uint16_t year, uint8_t month, uint8_t day)
{
    ...
    {
        days16 += daysArray[i-1];//pgm_read_byte(daysArray + i - 1);
    }
...
uint8_t DS1307::dow(uint16_t y, uint8_t m, uint8_t d)
{
    ...
    y -= m < 3;
    dow = ((y + y/4 - y/100 + y/400 + dowArray[m-1] /*pgm_read_byte(dowArray+(m-1))*/ + d) % 7);
